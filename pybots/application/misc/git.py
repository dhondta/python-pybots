...
class GitRecoveryBot(HTTPBot):
    ...
    def __extract_commits(self, output):
        ...
        :param output: output text, e.g. the content of .git/logs/HEAD
        ...
        if self.response.status_code == 200:
            ...
            if commit:
                try:  # decompress and read the downloaded commit object
                    ...
                    self.logger.warn("Parsing '{}' failed".format(fp))
    ...
    def __fetch_commits(self):
        ...
        try:
            ...
            self.logger.warn("Could not extract commits from logs/HEAD")
        try:
            with open(".git/refs/heads/master") as f:
                self.__fetch_file(f.read().strip(), True)
        except IOError:
            self.logger.warn("Could not extract commits from refs/heads/master")
    ...
    def checkout(self, branch="master"):
        ...
        out, err = execute(cmd)
        if "unable to read" in err:
            self.__extract_commits(out)
            self.__extract_commits(err)
            self.checkout(branch)
        else:
            for l in out.split('\n'):
                if len(l) > 0 and not l.startswith("Already on"):
                    fp = l.split('\t')[-1]
